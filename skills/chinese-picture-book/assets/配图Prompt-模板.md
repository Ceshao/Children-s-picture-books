...
> 本工程**手动出图**：绘画师产出本文件，由人粘贴到 ChatGPT/即梦等生成。

> **🔒 统一尺寸铁律**：所有页（封面、扉页、内页、满版高潮页）都用**同一个画布尺寸**（默认竖版 2:3，≈1024×1536），来自故事圣经的画风字段。每条 prompt 都带 `CANVAS` 行；高潮页靠**构图**铺满同一张画布制造气势，**绝不换更宽/横版尺寸**。若工具出图尺寸略有出入，排版成 PDF 前**统一裁成同一尺寸**。
...
STYLE: [从故事圣经的风格字段复制 STYLE/PALETTE 块]
OUTFIT: [书中固定服装，来自角色身份锚点]
...
> 若没有真实孩子照片（虚构角色），跳过照片，仅用角色锚点 + 主参考图锁定一致性。
...
SHOT:  <见下表，把镜头代码翻成英文>
SCENE: <场景>
ACTION:<这一页角色在做什么，来自画面脚本"画面必须讲清的事">
...
> 注：prompt 里加 `no text in image`，避免模型在图上自动生成文字（文字由排版时另放）。
...
SHOT: wide establishing shot, high angle, show the whole quiet park at sunrise, a winding path.
SCENE: a quiet neighborhood park at sunrise.
ACTION: no characters in frame — just the calm park and the path leading forward.
COMPOSITION: path leads from lower-left toward upper-right, inviting the eye forward.
Children's picture book illustration, single page, warm and child-friendly, no text in image.
```
> 这是满足"非人物页 ≥25%"的环境页；该页文字只配一个声音词或一句短句（或无字）。
